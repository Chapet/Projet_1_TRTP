...
pkt_status_code pkt_decode(const char *data, const size_t len, pkt_t *pkt) {
    ...
    ret_msg = pkt_set_window(pkt, (uint8_t) ((uint8_t) (*data) << 3u)
            >> 3u); // decode window field (crushing type & tr fields first)
    if (ret_msg != PKT_OK) {
        return ret_msg;
    }
...
ssize_t varuint_encode(uint16_t val, uint8_t *data, const size_t len) {
    ...
    if (len < 1 || nBytes == -1 ||
        (nBytes == 2 && len == 1)) { // len arg too small or error in predict_len or data conflict
        return -1;
    } else if (nBytes == 1) {
